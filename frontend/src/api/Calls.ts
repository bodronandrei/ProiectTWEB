...
api.interceptors.response.use(
    ...
    (error: AxiosError) => {
        if (error.response) {           
            console.error('Eroare de raspuns:', error.response.data);
        } else if (error.request) {           
            console.error('Niciun raspuns primit:', error.request);
        } else {           
            console.error('Erroare la configurarea cererii:', error.message);
        }      
